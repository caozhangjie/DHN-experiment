...
import random
import cv2
from alexnet import Alexnet, AdversarialNet
from utils import *
import shutil
...
class Net(object):
    def __init__(self, config):
        ...
        with tf.device(self.device):
            ...
            self.train_img = tf.placeholder(tf.float32, 
                [self.batch_size*2, 256, 256, 3])
            self.train_label = tf.placeholder(tf.float32, 
                [self.batch_size, self.n_class])
            self.ad_labels = tf.placeholder(tf.float32, [self.batch_size*2, 2])
            self.l = tf.placeholder(tf.float32, [])
            self.global_step = tf.Variable(0, trainable=False)
            ### Construct CNN
            self.cnn = Alexnet(self.model_weights, self.hash_bit_num)
            self.ad_net = AdversarialNet(4096)
            ### Construct train net
            train_img = self.preprocess_img(self.train_img, self.batch_size*2, True)
            with tf.variable_scope("cnn"):
                feature = self.cnn.extract(train_img)
                hash_bit = tf.tanh(tf.slice(feature, [0, 0], [self.batch_size, self.hash_bit_num]))
                self.hash_bit = hash_bit
                fc7_feature = self.cnn.fc7
            with tf.variable_scope("adnet"):
                ad_out = self.ad_net.extract(fc7_feature, self.l)
            self.lr_mult = self.cnn.lr_mult
            self.lr_mult_ad = self.ad_net.lr_mult
            self.lr_mult_all = {}
            for val in self.lr_mult:
                self.lr_mult_all[val] = self.lr_mult[val]
            for val in self.lr_mult_ad:
                self.lr_mult_all[val] = self.lr_mult_ad[val]


            log('setup', 'Construct Losses')
            ### pairwise cross entropy loss
            inner_product = tf.matmul(hash_bit, tf.transpose(hash_bit))
            #inner_product = tf.divide(float(self.hash_bit_num) / 2, tf.add(tf.reduce_sum(tf.square(tf.subtract(tf.reshape(self.hash_bit, [self.batch_size, 1, self.hash_bit_num]), tf.reshape(self.hash_bit, [1, self.batch_size, self.hash_bit_num]))), axis=2), 1.0))
            ...
            ### overall loss
            self.ad_loss = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(logits=ad_out, labels=self.ad_labels))
            self.loss = tf.add(self.ad_loss, tf.add(self.pairwise_loss, tf.multiply(self.quantization_loss, self.quantization_loss_weight)))
            ...
            grads_and_vars = opt.compute_gradients(self.loss, self.cnn.var_list+self.ad_net.var_list)

            ### apply gradients
            check = lambda var, w: var if var is not None else tf.zeros(tf.shape(w))
            self.train_op = opt.apply_gradients([
                (check(grads_and_vars[i][0], w) * self.lr_mult_all[w], w) for (i, w) in enumerate(self.cnn.var_list+self.ad_net.var_list)],
    ...
    def train(self, train_set, test_set):
        ...
        for i in xrange(self.max_iter):
            ...
            test_img, test_label = test_set.next_batch(self.batch_size)
            ad_label = np.vstack([np.tile([1., 0.], [self.batch_size, 1]),
                                   np.tile([0., 1.], [self.batch_size, 1])])
            all_img = np.concatenate((train_img, test_img), axis=0)
            lr = 0.00005 / ((1. + 0.001 * i)**0.75)
            _, loss, hash_bit, p_loss, q_loss, ad_loss = self.sess.run([self.train_op, self.loss, self.hash_bit, self.pairwise_loss, self.quantization_loss, self.ad_loss], feed_dict={self.train_img: all_img, self.train_label: train_label, self.ad_labels:ad_label, self.l:2. / (1. + np.exp(-10. * i/10000.)) - 1, self.lr:lr})
            #if (i + 1) % 100 == 0:
                #print hash_bit
            print "iter " + str(i+1) + ", loss " + str(loss) + ", pairwise_loss " + str(p_loss) + ", quantization_loss " + str(q_loss) + ", ad_loss " + str(ad_loss)
...
def train():
    ...
    net = Net(config)
    train_set = Dataset("./data/challenge/train_train_list.txt", config['n_class'])
    test_set = Dataset("./data/challenge/validation_train_list.txt", config['n_class'])
